...
def seed_into(con, data: dict, *, start_fresh: date) -> dict:
    """Create users, sections, metrics, targets, history, and display/goal-band
    settings from a seed_data-shaped dict, into whatever DB `con` points at.
    Also used by app.demo to build the throwaway demo database, which is why
    it takes the connection and the data instead of reading globals.

    Returns {"uid": {email: id}, "mid": {key: id},
             "creds": [(name, email, temp_pw)], "display_token": str}.
    The caller owns printing secrets and creating API tokens.
    """
    ...
    # Metrics without historical values start at start_fresh instead, so they
    # don't render months of fake stale cells.
    start_hist = min(history_weeks) if history_weeks else start_fresh
    ...
    return {"uid": uid, "mid": mid, "creds": creds, "display_token": display}
...
def main() -> None:
    ...
    with dbm.get_db() as con:
        ...
        res = seed_into(con, data, start_fresh=date(2026, 7, 6))
        admin_id = res["uid"][data["users"][0]["email"]]
        api_tok = new_api_token(con, "hermes", "read_write", admin_id)

        print("\n=== SEED COMPLETE - copy these now, they are not shown again ===")
        for name, email, pw in res["creds"]:
            print(f"  {name:<16} {email:<28} temp password: {pw}")
        print(f"\n  TV display URL path: /display?token={res['display_token']}")
