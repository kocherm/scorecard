...
def test_sweep_routes_each_user_through_their_channel(env):
    ...
    tg_text = tg[1]["json"]["text"]
    assert "Or, you can reply here like" in tg_text        # two-way
    # Telegram is sent without a parse_mode, so it gets the bare URL rather
    # than <url|label> syntax it would render literally.
    assert "https://score.example.com/checkin?t=" in tg_text
    assert "|Update your numbers here>" not in tg_text

    teams = by_host(calls, "teams.example.com")[0]
    text = teams[1]["json"]["text"]
    assert text.startswith("Tom Teams - ")                 # channel post names owner
    assert "Or, you can reply here like" not in text       # link-only
    assert "[Update your numbers here](https://score.example.com/checkin?t=" in text
    ...
    assert "Or, you can reply here like" in tw[1]["data"]["Body"]
