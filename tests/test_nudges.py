...
def test_nudge_dm_lists_missing_with_targets_and_link(env):
    ...
    # Slack renders <url|label>, so the link is a phrase rather than a wall of
    # query string - and it leads, ahead of the typed-reply shortcut.
    assert "|Update your numbers here>" in text
    assert "<https://score.example.com/checkin?t=" in text
    assert text.index("Update your numbers here") < text.index("Still missing:")
    assert 'Or, you can reply here like "1: 12, 2: G"' in text
    with dbm.get_db() as con:
