...
def link(channel: str, url: str, label: str) -> str:
    """Render a link the way this channel actually displays it. Slack and
    Google Chat take <url|label>; Teams takes markdown. Telegram is sent
    without a parse_mode and SMS has no markup at all, so both get the bare
    URL - a channel that shows the raw syntax is worse than one that shows a
    long link."""
    if channel in ("slack", "gchat"):
        return f"<{url}|{label}>"
    if channel == "teams":
        return f"[{label}]({url})"
    return url
